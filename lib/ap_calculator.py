...
def create_lineset_from_box_corners(box_corners, color=[0, 0, 1]):
    # corners are assumed to be in the following order:
    # 0: front bottom left
    ...
    lineset.points = o3d.utility.Vector3dVector(box_corners)
    lineset.lines = o3d.utility.Vector2iVector(lines)
    lineset.colors = o3d.utility.Vector3dVector([color for i in range(len(lines))])

    return lineset

def visualize(gt_box_corners, gt_box_present, point_cloud, predicted_box_corners):
    # vis = o3d.visualization.Visualizer()
    # vis.create_window()

    for batch_idx in range(4): # iterate over batch
        # visualize point cloud
        pc = point_cloud[batch_idx]
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(pc[:,:3])
        pcd.colors = o3d.utility.Vector3dVector(pc[:,3:6])
        o3d.io.write_point_cloud(f"pointcloud_{batch_idx}.ply", pcd)
        # vis.add_geometry(pcd)

        # # visualize ground truth boxes
        # for i in range(128): # iterate over boxes
        #     if gt_box_present[batch_idx, i] == 1: # if box is present
        #         lineset = create_lineset_from_box_corners(gt_box_corners[batch_idx, i], color=[0, 0, 1]) # blue color for GT boxes
        #         vis.add_geometry(lineset)

        # # visualize predicted boxes
        # for i in range(128): # iterate over boxes
        #     lineset = create_lineset_from_box_corners(predicted_box_corners[batch_idx, i], color=[1, 0, 0]) # red color for predicted boxes
        #     vis.add_geometry(lineset)

    # vis.run()
    # vis.capture_screen_image("pointcloud_with_boxes.png")
    # vis.destroy_window()
...
class APCalculator(object):
    ...
    def step_meter(self, outputs, targets):
        ...
        self.step(
            ...
            objectness_probs=outputs["objectness_scores"].squeeze(-1),
    ...
    def step(
        ...
        Convert GT boxes
        """
        # CHECKPOINT 26/05 make visualization
