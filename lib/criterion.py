...
class SetCriterion(nn.Module):
    ...
    def loss_sem_cls(self, outputs, targets, assignments):
        ...
        loss = F.cross_entropy(
            ...
            gt_box_label,
            self.semcls_percls_weights.to(device=pred_logits.device),
            reduction="mean",
        )
    ...
    def single_output_forward(self, outputs, targets):
        # visualize_boxes(gt_box_corners.detach().cpu().numpy()[:, :10], targets["box_label_mask"].detach().cpu().numpy()[:, :10], targets["point_clouds"].detach().cpu().numpy(), outputs["box_corners"].detach().cpu().numpy()[:, :3], 40, -120)
        # gt_center_list = targets['center_label'].cpu().numpy()  # (B,3)
        # gt_heading_class_list = targets['heading_class_label'].cpu().numpy()  # B
        # gt_heading_residual_list = targets['heading_residual_label'].cpu().numpy()  # B
        # gt_size_class_list = targets['size_class_label'].cpu().numpy()  # B
        # gt_size_residual_list = targets['size_residual_label'].cpu().numpy()  # B,3
        # batch_size = gt_center_list.shape[0]
        # gt_box_corners = torch.zeros((batch_size, 128, 8, 3), dtype=torch.float32, device=outputs['box_corners'].device)
        # for i in range(batch_size):
        #     gt_obb_batch = self.dataset_config.param2obb_batch(gt_center_list[i][:, 0:3], gt_heading_class_list[i],
        #                                         gt_heading_residual_list[i],
        #                                         gt_size_class_list[i], gt_size_residual_list[i])
        #     gt_bbox_batch = get_3d_box_batch(gt_obb_batch[:, 3:6], gt_obb_batch[:, 6], gt_obb_batch[:, 0:3])
        #     gt_box_corners[i] = torch.tensor(gt_bbox_batch, dtype=torch.float32, device=outputs['box_corners'].device)
        ...
        center_dist = torch.cdist(
            outputs["center"], targets["gt_box_centers_normalized"], p=1
        )
    ...
    def forward(self, data_dict):
        # visualize_boxes(data_dict["gt_box_corners"].detach().cpu().numpy()[:, :10], data_dict["box_label_mask"].detach().cpu().numpy()[:, :10], data_dict["point_clouds"].detach().cpu().numpy(), data_dict["box_corners"].detach().cpu().numpy()[:, :3], 40, -120)
        nactual_gt = data_dict["box_label_mask"].sum(axis=1).long()
        ...
        loss, loss_dict = self.single_output_forward(data_dict, data_dict)
