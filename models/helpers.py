...
def get_clones(module, N, language_module=None):
    if language_module is not None:
        return nn.ModuleList([copy.deepcopy(module) for i in range(N - 2)] + [copy.deepcopy(language_module) for i in range(2)])
    else:
